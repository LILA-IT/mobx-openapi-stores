...
[![npm downloads](https://img.shields.io/npm/dm/mobx-openapi-stores.svg)](https://www.npmjs.com/package/mobx-openapi-stores)
...
- **Type-Safe**: Distributive `ApiMethodName` / `ApiMethodArgs` helpers for OpenAPI client unions.
- **Reactive**: Built with MobX for efficient and easy-to-manage state.
- **Modular**: Distinct store classes for single entities, collections, paginated lists, CRUD, and keyed objects.
- **Extensible**: Base stores can be easily extended to add application-specific logic.
- **Loading State Management**: Integrated loading indicators, including `disableLoading` for prefetch.
- **Simplified API Calls**: `apiCall` preserves generated client `this` binding.
- **Error Handling**: Errors are processed and logged to the console.

## Repository

- **Public mirror / npm source metadata**: [GitHub](https://github.com/LILA-IT/mobx-openapi-stores)
- Primary development, MRs, tests, and version tagging run on an internal Git host (not linked here).
- **npm publishes** use [trusted publishing](https://docs.npmjs.com/trusted-publishers) from GitHub Actions (`.github/workflows/publish.yml`) on `v*` tags. Security scanning stays on the internal CI.
...
Peer dependency: **MobX 6+** (uses `makeObservable`).
...
6.  **`PaginationStore`**: Page cache, prefetch, and list invalidation for paginated endpoints.
7.  **`ObjectStore`**: Manages a dictionary-like observable object, useful for grouping entities.
